...
public class Department {
	...
	public static final String D_manager = "manager";

	@DAttr(name = "id", id = true, auto = true, length = 3, mutable = false, optional = false, type = Type.Integer)
	private int id;
	private static int idCounter;

	@DAttr(name = D_name, type = Type.String, length = 20, optional = true, cid = true)
	private String name;
	
	@DAttr(name = D_manager, type = Type.Domain, length = 20, optional = true)
	private Manager manager;

	@DAttr(name="employees",type=Type.Collection,optional = false,
		      serialisable=false,filter=@Select(clazz=Employee.class))
		  @DAssoc(ascName="department-has-employees",role="department",
	...
	public Department(@AttrRef("name") String departmentName) {
		this(null, departmentName, null);
	}
	
	// from object form: Student is included
		@DOpt(type = DOpt.Type.ObjectFormConstructor)
		public Department(@AttrRef("name") String departmentName, @AttrRef("manager") Manager manager) {
			this(null, departmentName, manager);
		}

		// from data source
		@DOpt(type = DOpt.Type.DataSourceConstructor)
		public Department(@AttrRef("id") Integer id, @AttrRef("name") String departmentName) {
			this(id, departmentName, null);
		}

	// based constructor (used by others)
	public Department(Integer id, String departmentName, Manager manager) {
		this.id = nextId(id);
